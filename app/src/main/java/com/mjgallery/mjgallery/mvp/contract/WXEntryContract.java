...
public interface WXEntryContract {
    ...
    interface View extends IView {
        void getLogin(LoginResponse baseResponse);

        void getSendCode(BaseResponse baseResponse);

        void getRegistern(BaseResponse baseResponse);
        void onError();
    }

    //Model层定义接口,外部只需关心Model返回的数据,无需关心内部细节,即是否使用缓存
    interface Model extends IModel {
        Observable<LoginResponse> getLogin(Map<String, Object> map);

        Observable<BaseResponse> getSendCode(Map<String, Object> map);

        Observable<BaseResponse> getRegistern(Map<String, Object> map);
    }
}
