...
public class MyAccountPresenter extends BasePresenter<MyAccountContract.Model, MyAccountContract.View> {
    ...
    public void requestUserData(Map<String, Object> map) {
        mRootView.showLoading();
        mModel.getMyAccountUser(map)
                ...
                .subscribe(new ErrorHandleSubscriber<BaseResponse<MyAccountUserBean>>(mErrorHandler) {
                    @Override
                    public void onNext(BaseResponse<MyAccountUserBean> listBaseResponse) {
                        if (listBaseResponse.getResult() != null) {
                            EventBus.getDefault().post(new RemoveTokenEvent(listBaseResponse.getCode()));
                            mRootView.onMyAccountUser(listBaseResponse.getResult());
    ...
    public void requestData(Map<String, Object> map) {
        ...
        mModel.getAndroidGetAccount(map)
                .subscribeOn(Schedulers.io())
                .retryWhen(new RetryWithDelay(3, 15))//遇到错误时重试,第一个参数为重试几次,第二个参数为重试的间隔
                .subscribeOn(AndroidSchedulers.mainThread())
                .observeOn(AndroidSchedulers.mainThread())
                ...
                .compose(RxLifecycleUtils.bindToLifecycle(mRootView))//使用 Rxlifecycle,使 Disposable 和 Activity 一起销毁
                .subscribe(new ErrorHandleSubscriber<BaseResponse<AndroidGetAccountXBean>>(mErrorHandler) {
                    @Override
                    public void onNext(BaseResponse<AndroidGetAccountXBean> listBaseResponse) {
                        if (listBaseResponse.getResult() != null) {
                            EventBus.getDefault().post(new RemoveTokenEvent(listBaseResponse.getCode()));
                            mRootView.onAndroidGetAccount(listBaseResponse.getResult());
