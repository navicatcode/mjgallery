...
public class MessagePicturesLayout extends FrameLayout implements View.OnClickListener {
    ...
    private List<Uri> mThumbDataList = new ArrayList<>();
    ...
    public void set(List<MyShowBean.ShowPictureDoListBean> urlThumbList) {
        mDataList.clear();
        mThumbDataList.clear();
        for (int i = 0; i < urlThumbList.size(); i++) {
            Uri uri = Uri.parse(urlThumbList.get(i).getFilePath());
            mThumbDataList.add(uri);
            mDataList.add(uri);
